...
我们提供了一个包含历史文献引用信息的简单数据集。每篇文献都分配有一个唯一标识符（UUID），以便识别。为了适应链接预测的下游任务，我们在数据集中直接使用了所有可能的正样本，并计算出相同数量的不存在的链接作为负样本，供模型训练和验证使用。

### 数据准备

为了准备数据集，您需要收集文章及其引用信息，并确保每篇文章都有一个唯一的标识符，如 UUID 或 DOI 号。在数据预处理阶段，可以暂时使用文章的详细引用信息（例如，全名）作为唯一标识符。
...
- [x] 特征工程：对类别型特征（如作者名）使用独热编码，并设计基于引用数量的特征等。
